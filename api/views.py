...
class JoinRoom(APIView):
    ...
    def post(self, request, format=None):
        ...
        return Response({'Bad Request':'Invalid post data,did not finh code key'},status=status.HTTP_400_BAD_REQUEST)
...
class LeaveRoom(APIView):
    def post(self, request, format=None):
        ...
        room =Room.objects.filter(host=is_host)
        if len(room)!=0:
            room=room[0]
            room.delete()
...
class UpdateRoom(APIView):
    ...
    def patch(self, request , format=None):
        ...
        if serializers.is_valid():
            ...
            room=queryset[0]
            used_id = self.request.session.session_key
            if room.host != used_id:
